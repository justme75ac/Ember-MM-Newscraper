...
#Region "Fields"
    ...
    Private _Name As String = "AEBN_Trailer"
...
#Region "Methods"
    ...
    Function Scraper_Movie(ByRef DBMovie As Database.DBElement, ByVal Type As Enums.ModifierType, ByRef TrailerList As List(Of MediaContainers.Trailer)) As Interfaces.ModuleResult Implements Interfaces.ScraperModule_Trailer_Movie.Scraper
        logger.Trace("[AEBN_Trailer] [Scraper_Movie] [Start]")

        Dim tAEBNID As String = String.Empty

        If Not String.IsNullOrEmpty(DBMovie.Movie.AEBN) Then
            Dim _scraper As New AEBNs.Scraper()

            TrailerList = _scraper.GetTrailers(DBMovie.Movie.AEBN)
        End If
        logger.Trace("[AEBN_Trailer] [Scraper_Movie] [Done]")
