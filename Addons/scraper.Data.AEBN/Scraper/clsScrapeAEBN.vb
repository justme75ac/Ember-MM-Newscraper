        ...
        Private Function GetPlotAndOutline(ByVal HTML As String) As Results
            ...
            Try
                If Not String.IsNullOrEmpty(HTML) Then
                    ...
                    Dim descPattern As String = "<div class=""dts-section-page-detail-description-body"">(?<DESCRIPTION>.*?)</div>"
                    ...
                    'End If
                    'End If
                    strPlot = "PLOT: " + strDescription
                    strOutline = "OUT: " + strDescription
                End If
            Catch ex As Exception
                logger.Error(ex, New StackFrame().GetMethod().Name)
            End Try
        ...
        Private Function SearchMovie(ByVal strTitle As String, ByVal strYear As String) As String
            ...
            Try
                If Not String.IsNullOrEmpty(strTitle) Then
                    ...
                    Dim sHTTP As New HTTP
                    Dim HTML As String = sHTTP.DownloadData(String.Concat("https://gay.aebn.com/gay/search/movies/page/1?queryType=Free+Form&query=""", strTitle, """&criteria={%22sort=Relevance%22}"))
                    sHTTP = Nothing
                    ...
                    If Not String.IsNullOrEmpty(HTML) Then
                        ...
                        Dim filterPattern As String = "<div class=\""dts-search-results-container"">(?<RESULTS>.*?)<script>"
                        ...
                        If Not String.IsNullOrEmpty(strSearchResults) Then
                            ...
                            Dim resPattern As String = "<div id=""dtsImageOverlayContainer-.*?"" class=""dts-image-overlay-container"" style=""position: relative;"">.*?<a href=""(?<URL>.*?)"".*?>.*?title=""(?<TITLE>.*?)"".*?<\/a>"
                            ...
                            If resResult.Count = 1 OrElse (filterResult.Count > 0) Then
                                If strURL = String.Empty OrElse strURL.Contains("https://gay.aebn.com") = False Then
                                    strURL = String.Concat("https://gay.aebn.com", resResult.Item(0).Groups("URL").Value).Trim
                                End If
